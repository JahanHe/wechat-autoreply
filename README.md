...
[![Release v0.3.5](https://img.shields.io/badge/release-v0.3.5-111827)](https://github.com/JahanHe/wechat-autoreply/releases/tag/v0.3.5)
...
| 你关心的问题 | 当前答案 |
| --- | --- |
| 下载入口在哪里？ | 在 [GitHub Releases v0.3.5](https://github.com/JahanHe/wechat-autoreply/releases/tag/v0.3.5)，不是 Packages |
...
| 系统 | 下载 |
| --- | --- |
| macOS Apple Silicon | [wechat-autoreply-macos-arm64.dmg](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.5/wechat-autoreply-macos-arm64.dmg) |
| Windows 安装版 | [wechat-autoreply-windows-setup.exe](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.5/wechat-autoreply-windows-setup.exe) |
| Windows 便携版 | [wechat-autoreply-windows-portable.exe](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.5/wechat-autoreply-windows-portable.exe) |

发布说明：[docs/release-notes/v0.3.5.md](docs/release-notes/v0.3.5.md)

首次打开后先进入“初始化”：

| 步骤 | 位置 | 做什么 |
| --- | --- | --- |
| 1 | 主控制台 > 初始化 | 填 DeepSeek API Key、企业微信 Webhook，点击“登录并自动接入”完成润宇登录 |
...
| 查看当前发行版的富文本说明 | [docs/release-notes/v0.3.5.md](docs/release-notes/v0.3.5.md) |
...
推荐在“初始化”或“判断库”页点击“登录并自动接入”。应用使用独立的 `persist:runyu-auth` 会话保存这台电脑的网页登录状态，自动读取 `session_token`，写入本机配置，并执行真实查询验证。重启后会自动恢复和复验；登录过期时点击“重新登录”。手工复制 Cookie 仅用于故障备用。`401` 表示登录已过期，`403` 表示账号缺少工具权限，`404` 通常是 Base URL 或网络解析问题。
