...
| 你关心的问题 | 当前答案 |
| --- | --- |
| 下载入口在哪里？ | 在 [GitHub Releases v0.3.9](https://github.com/JahanHe/wechat-autoreply/releases/tag/v0.3.9)，不是 Packages |
...
| 步骤 | 位置 | 做什么 |
| --- | --- | --- |
| 1 | 系统设置 > 初始化 | 填 DeepSeek API Key 和企业微信 Webhook |
| 2 | 系统设置 > 初始化 | 点“打开登录网页”，5 分钟内完成润宇网页登录 |
| 3 | 系统设置 > 初始化 | 点“我已登录，获取凭证”，强制远端查询并自动初始化 10 条引用缓存 |
| 4 | 系统设置 > 初始化 | 点“保存并自检”，检查 AI、Webhook、判断库、规则库和长期运行状态 |
| 5 | 客服工作台 | 微信扫码登录并选中客服会话 |
...
主控制台现在只有四个一级入口，二级功能在页面顶部切换：

| 一级入口 | 二级功能 | 能做什么 |
| --- | --- | --- |
| 客服工作台 | 客服页 | 保留微信小店原客服页，登录、选会话和聊天都在这里 |
| 回复中心 | 规则库、Bot策略、API风格、判断库 | 编辑文字/图片/商品/文件规则，调整承接语、兜底回复、DeepSeek、回复风格和判断库 |
| 运行监控 | 总览、日志 | 查看当前步骤、Bot、AI、客服页、悬浮窗、回复来源、判断库 Trace 和异常记录 |
| 系统设置 | 初始化、Webhook、悬浮窗、说明 | 首次配置 Key/Webhook，网页登录判断库，设置通知规则，打开/隐藏固定悬浮窗，查看帮助 |
...
| 查看当前发行版的富文本说明 | [docs/release-notes/v0.3.9.md](docs/release-notes/v0.3.9.md) |
| 使用旧迁移包或源码方式安装 | [PORTABLE_INSTALL.md](PORTABLE_INSTALL.md) |
| Windows 手动安装和任务计划说明 | [WINDOWS_INSTALL.md](WINDOWS_INSTALL.md) |

## 规则长什么样

动作规则可以直接在回复中心 > 规则库里用卡片编辑。高级 JSON 只作为批量排查和迁移入口。例子：
...
图片和文件动作的 `path` 在主控制台 > 回复中心 > 规则库里可以直接操作：手动编辑、点“选择/替换”改成新图片或文件，或点“打开位置”直达当前文件所在目录。
