...
[![Release v0.3.3](https://img.shields.io/badge/release-v0.3.3-111827)](https://github.com/JahanHe/wechat-autoreply/releases/tag/v0.3.3)
...
| 你关心的问题 | 当前答案 |
| --- | --- |
| 下载入口在哪里？ | 在 [GitHub Releases v0.3.3](https://github.com/JahanHe/wechat-autoreply/releases/tag/v0.3.3)，不是 Packages |
| 首次运行要配什么？ | DeepSeek API Key、企业微信机器人 Webhook、外部判断库 Session Cookie，然后扫码登录微信小店 |
...
| 系统 | 下载 |
| --- | --- |
| macOS Apple Silicon | [wechat-autoreply-macos-arm64.dmg](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.3/wechat-autoreply-macos-arm64.dmg) |
| Windows 安装版 | [wechat-autoreply-windows-setup.exe](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.3/wechat-autoreply-windows-setup.exe) |
| Windows 便携版 | [wechat-autoreply-windows-portable.exe](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.3/wechat-autoreply-windows-portable.exe) |

发布说明：[docs/release-notes/v0.3.3.md](docs/release-notes/v0.3.3.md)

首次打开后先进入“初始化”：

| 步骤 | 位置 | 做什么 |
| --- | --- | --- |
| 1 | 主控制台 > 初始化 | 填 DeepSeek API Key、企业微信 Webhook、外部判断库 Session Cookie |
...
| 页面 | 能做什么 |
| --- | --- |
| 初始化 | 首次配置 Key、Webhook、判断库 Cookie，并自动做功能安全自检 |
...
| 查看当前发行版的富文本说明 | [docs/release-notes/v0.3.3.md](docs/release-notes/v0.3.3.md) |
...
Cookie 获取方式：用 Chrome 登录 `https://runyuai.zhiduoke.com.cn`，从 Application > Cookies > `runyuai.zhiduoke.com.cn` 复制 `session_token`。不要复制 Session Storage 里的 token。`401` 表示 Cookie 过期或没有权限，`404` 通常是 Base URL 或网络解析问题，v0.3.3 会自动规整 Base URL 并使用 `curl --resolve` 备用线路。
