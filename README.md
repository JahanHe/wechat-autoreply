...
[![Release v0.3.6](https://img.shields.io/badge/release-v0.3.6-111827)](https://github.com/JahanHe/wechat-autoreply/releases/tag/v0.3.6)
...
| 你关心的问题 | 当前答案 |
| --- | --- |
| 下载入口在哪里？ | 在 [GitHub Releases v0.3.6](https://github.com/JahanHe/wechat-autoreply/releases/tag/v0.3.6)，不是 Packages |
| 首次运行要配什么？ | DeepSeek API Key、企业微信机器人 Webhook；润宇判断库和微信小店分别完成一次网页登录 |
...
| 系统 | 下载 |
| --- | --- |
| macOS Apple Silicon | [wechat-autoreply-macos-arm64.dmg](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.6/wechat-autoreply-macos-arm64.dmg) |
| Windows 安装版 | [wechat-autoreply-windows-setup.exe](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.6/wechat-autoreply-windows-setup.exe) |
| Windows 便携版 | [wechat-autoreply-windows-portable.exe](https://github.com/JahanHe/wechat-autoreply/releases/download/v0.3.6/wechat-autoreply-windows-portable.exe) |

发布说明：[docs/release-notes/v0.3.6.md](docs/release-notes/v0.3.6.md)

首次打开后先进入“初始化”：

| 步骤 | 位置 | 做什么 |
| --- | --- | --- |
| 1 | 主控制台 > 初始化 | 填 DeepSeek API Key 和企业微信 Webhook |
| 2 | 主控制台 > 初始化 | 点“打开登录网页”，5 分钟内完成润宇网页登录 |
| 3 | 主控制台 > 初始化 | 点“我已登录，获取凭证”，强制远端查询并自动初始化 10 条引用缓存 |
| 4 | 主控制台 > 初始化 | 点“保存并自检”，检查 AI、Webhook、判断库、规则库和长期运行状态 |
| 5 | 主控制台 > 客服页映射 | 微信扫码登录并选中客服会话 |
...
| 页面 | 能做什么 |
| --- | --- |
| 初始化 | 首次配置 Key、Webhook，通过网页登录自动接入判断库，并做功能安全自检 |
| 客服页映射 | 保留微信小店原客服页，登录、选会话和聊天都在这里 |
| 总览状态 | 查看当前步骤、最近步骤、Bot、AI、Webhook、客服页、悬浮窗和回复记录 |
| Bot 接管 | 开启/暂停 Bot，调整承接语、超时、图片和页面动作能力 |
| API 接入 | 配 DeepSeek Key、模型、Base URL、风格、知识库和测试回复 |
| 判断库 | 登录 Runyu、自动获取本机凭证，并配置来源、类型、下载、刷新和测试查询 |
...
| 查看当前发行版的富文本说明 | [docs/release-notes/v0.3.6.md](docs/release-notes/v0.3.6.md) |
...
判断库用于复杂问题的精准补充：Bot 会先请求 AI/判断库，15 秒还没结果才轮换发送承接语，60 秒还没有可用答案才轮换发送兜底回复。承接语和兜底都不是最终完成标记；AI 稍后返回时仍会继续发送详细答案。

| 配置项 | 说明 |
| --- | --- |
| `RUNYU_WEB_COOKIE` | 通过应用内 Runyu 登录自动获取并只写本机 `.env`；手工填写仅作为备用 |
| `RUNYU_WEB_BASE_URL` | 只填 `https://runyuai.zhiduoke.com.cn`；如果粘了完整 `/api/sync/judgments/query`，程序会自动规整为域名 |
...
推荐使用页面内的四步引导：打开登录网页、确认网页已登录、点击“我已登录，获取凭证”、等待首次引用库初始化。应用使用独立的 `persist:runyu-auth` 会话保存这台电脑的登录状态；只有远端真实查询成功才显示连接成功，本地缓存不会冒充鉴权通过。首次验证后自动查询 10 条并写入本地缓存。

登录页提供 5 分钟倒计时、Cookie 检测、自检、重新登录、错误码复制和最近凭证记录。后台会定期复验；失败时 Webhook 通知错误，恢复后再通知恢复成功。手工复制 Cookie 仅用于故障备用。

| 错误码 | 处理方式 |
| --- | --- |
| `RUNYU_SESSION_TOKEN_NOT_FOUND` | 返回登录窗口确认已经登录并加载完成，再点“我已登录，获取凭证” |
| `RUNYU_AUTH_EXPIRED` / HTTP 401 | 点“重新登录”，用新登录替换旧 Token |
| `RUNYU_PERMISSION_DENIED` / HTTP 403 | 改用有判断库权限的账号 |
| `RUNYU_API_404` / HTTP 404 | Base URL 只保留域名，保存后重新自检 |
| `RUNYU_NETWORK_FAILED` / `RUNYU_REQUEST_TIMEOUT` | 检查网络、代理和防火墙后重试，无需先清除 Cookie |
| `RUNYU_BOOTSTRAP_EMPTY` | 点“初始化引用库”重试，并复制错误信息和最近记录反馈 |
