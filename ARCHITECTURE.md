...
仓库默认只长期保留 `main` 和仍在处理的活动分支。`codex/*` 这类临时优化分支只用于一次明确任务或一个 PR，不作为长期版本线。

临时分支清理规则：

- PR 合并后，必须删除对应远端分支和本地分支。
- PR 关闭且确认不再继续时，必须删除对应远端分支和本地分支。
- 已经被 `main` 吸收的历史优化分支不得继续保留。
- 版本发布以 tag 和 GitHub 发布页为准，不用长期分支表示版本。
- 删除分支前必须确认没有打开的 PR、未推送提交或仍需继续移植的有效修复。

正常状态下，远端分支应只看到 `main` 和当前仍有打开 PR 的工作分支。
