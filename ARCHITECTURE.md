...
小店AI客服是一个微信小店客服 Electron 桌面自动回复工具。它把微信小店客服页嵌入桌面应用，通过本地规则、本机知识资料、AI API、页面动作和企业微信 Webhook，辅助店铺客服长期值守。
...
以后改动以下能力时，必须同步对应文档：Electron 窗口、菜单、托盘、悬浮窗、回复决策顺序、规则匹配、AI API、外部知识同步、本机索引、访问凭证、Webhook、通知补发、回复记录、配置文件、运行目录、打包文件、安全边界、发布流程和安装包命名。
...
- 本机回复中转服务负责检索本机资料、组装上下文、调用远方 AI API、审核回复、承接语和等待语。
- 生产回复只读取本机索引，不允许访问外部知识源。外部服务只用于授权、下载和增量同步；凭证与同步缓存只保存在本机运行目录。
- 本机索引统一覆盖公开 `knowledge-base`、手动知识、参考回复和已下载的外部同步资料，并保留来源标识。
...
| `src/` | CLI、AI 客户端、规则匹配、本机知识索引、外部同步、文本工具 | 共享业务逻辑优先放这里，避免 desktop 和 extension 重复实现；生产检索不得发起外部网络请求 |
...
5. 校验 AI API、Webhook、外部同步状态、本机索引、客服页登录和 Bot 心跳。
6. 窗口关闭默认隐藏，彻底退出必须停止后台服务并保存运行状态。

### 登录与授权

微信小店登录发生在客服页。外部知识同步使用独立 Electron 持久会话保存授权状态。授权必须通过同步接口验证，不能只根据本地凭证存在判断成功。验证成功后下载资料到本机同步缓存，再重建本机索引。Cookie 失效只影响后续更新，不得影响上一次成功下载的资料参与生产回复。
...
6. 匹配独立图片规则。
7. 匹配文字规则。
8. 未命中规则时，检索本机知识索引并调用 AI API；此步骤禁止实时查询外部知识源。
9. AI 超时后发送承接语或兜底语。
...
- 窗口、菜单、托盘、悬浮窗入口变化。
- 回复决策顺序、规则格式、动作类型变化。
- AI API、本机知识索引、外部知识同步、Webhook、控制接口变化。
- 运行目录、配置文件、安装包命名变化。
