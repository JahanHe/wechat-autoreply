import { summarizeAction, summarizeActions, summarizeActionText } from "./action-utils.js";
...
(() => {
  ...
  function messagePlaceholder(kind, detail = "") {
    return buildMessagePlaceholder(kind, detail);
  }
  ...
  function exportTask(task = {}) {
    return {
      ...
      customerText: clip(task.customerText, 180),
      route: task.route,
      status: task.status,
  ...
  function rememberCustomerMessage(task, message) {
    ...
    memory.recentCustomerMessages = [
      ...(Array.isArray(memory.recentCustomerMessages) ? memory.recentCustomerMessages : []),
      {
        ...
        text: clip(String(message?.text || task.customerText || ""), 240),
        at: Date.now()
      }
    ].slice(-20);
  ...
  function completeMemoryTask(task, completion = {}) {
    ...
    memory.completionResults = [
      ...(memory.completionResults || []),
      {
        ...
        reason: clip(completion.reason || task.completion?.reason || "", 160),
        at: Date.now()
      }
    ].slice(-40);
    if ((completion.result || task.completion?.result) === "solved") {
      memory.solvedTopics = uniqueTail([...(memory.solvedTopics || []), clip(task.customerText, 80)], 40);
    }
    persistCustomerMemories();
    reportCustomerMemory(memory);
  }
