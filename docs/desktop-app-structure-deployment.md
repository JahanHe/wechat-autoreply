...
7. 再匹配文字规则 `rules`。
8. 都没有命中时，本机回复中转服务检索本机自建资料和已下载的外部同步资料，再请求远方 AI API；生产回复不访问外部知识源。
9. 60 秒仍无最终答案时只标记延迟、写日志和通知，不默认给客户发送固定兜底。
...
3. 第一次打开后，在主控制台 > 初始化 填 DeepSeek API Key、企业微信 Webhook；如需私有资料，在“外部知识同步”完成授权和首次下载。
4. 点“保存并自检”，确认 AI API、Webhook、本机索引、外部同步、规则库和长期运行状态通过。
5. 登录微信小店客服页。
...
2. 第一次打开后，在主控制台 > 初始化 填 DeepSeek API Key、企业微信 Webhook；如需私有资料，在“外部知识同步”完成授权和首次下载。
3. 点“保存并自检”，确认 AI API、Webhook、本机索引、外部同步、规则库和长期运行状态通过。
4. 登录微信小店客服页。
5. 确认主控制台 > 规则库里已有默认文字规则和页面动作规则。
6. 悬浮窗显示“接管开启”后即可运行。

外部知识同步使用独立 Electron 持久会话 `persist:runyu-auth`。每台电脑首次使用时打开配置页完成授权，程序获取访问凭证并验证同步权限，然后把选择的资料下载到本机独立缓存并重建索引。生产 `/reply` 不使用该凭证，也不访问外部服务器。配置监控提供 5 分钟倒计时、错误码、最近凭证记录、下载进度和周期自检。`401` 时重新授权，`403` 表示账号缺少权限，`404` 先检查 Base URL。同步失败继续使用上一次成功下载的数据；恢复或同步成功后发送恢复通知。
...
- 点击“彻底关闭”需要 8 秒内点第二次。
- 彻底关闭后会停止自动回复、Webhook、悬浮窗和本机回复中转服务。
- 重新双击应用才会恢复。
