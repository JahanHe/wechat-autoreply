...
判断库使用独立的 Electron 持久会话 `persist:runyu-auth`。每台电脑依次完成“打开登录网页”“确认已登录”“获取凭证”“初始化引用库”；程序读取 `session_token`、保存到本机，并通过远端强制查询验证，不能用本地缓存代替鉴权。登录监控提供 5 分钟倒计时、错误码、最近凭证记录和周期自检。返回 `401` 时点击“重新登录”；`403` 表示账号缺少权限；`404` 先检查 Base URL；网络恢复或重新登录成功后会发送恢复通知。
