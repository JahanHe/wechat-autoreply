...
外部知识库网络调用模式使用独立的 Electron 持久会话 `persist:runyu-auth`。每台电脑依次完成“打开网络配置页”“确认配置完成”“获取访问凭证”“初始化本地缓存”；程序读取访问凭证、保存到本机，并通过远端强制查询验证，不能用本地缓存代替鉴权。本地导入模式可以跳过网络凭证。配置监控提供 5 分钟倒计时、错误码、最近凭证记录和周期自检。返回 `401` 时点击“重新配置”；`403` 表示账号缺少权限；`404` 先检查 Base URL；网络恢复或重新配置成功后会发送恢复通知。
