...
| 3 | 设置 > 初始化 | 授权成功后程序会自动获取访问凭证并验证；如果自动检测失败，再点“获取访问凭证” |
| 4 | 设置 > 初始化 | 点“保存并自检”，检查 AI、Webhook、外部知识库、规则库和长期运行状态 |
| 5 | 工作台 | 微信扫码登录小店客服 |

如果三项配置已经齐全，程序会直接进入客服页映射；如果换电脑后缺任意一项，会再次打开初始化页。

外部知识库网络调用模式通过应用内配置窗口接入。每台电脑第一次使用时点击“打开网络配置页”并完成授权。程序会检测新访问凭证、保存到本机、强制查询远端接口，再下载首次 10 条引用数据；如果自动检测失败，再手动点击“获取访问凭证”。只有真实查询成功才显示连接成功，本地缓存不会造成误判。

配置监控会显示 5 分钟倒计时、凭证检测、远端验证、缓存初始化、错误码和最近凭证记录。访问凭证过期时点“重新配置”；网络问题处理后点“检查访问凭证”；查询成功但缓存为空时点“初始化本地缓存”。验证和初始化成功后配置窗口会自动关闭。错误和恢复都会通过 Webhook 通知。手工粘贴访问凭证只作为备用，Base URL 保持服务域名即可。
...
| 外部知识库显示访问凭证过期 | 当前电脑的访问凭证已失效 | 点“重新配置”，完成授权后等待自动验证；如果没有自动检测，再点“获取访问凭证” |
